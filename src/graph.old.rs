use crate::{
    connection::{BackConnection, ConnectedDocument, ConnectionTarget},
...
struct PathData {
    ...
    /// The parsed document in this path. This contains all details about all nodes in the
    /// document, including alll parsed connections, and can be used to fully reconstruct it.
    ///
    /// This will be [`Some`] for paths which have just been parsed or re-parsed, and which have
    /// yet to have their connection titles checked and possibly rewritten. Holding this also
    /// allows us to enforce UUID consistency when we have to generate a UUID for a node in
    /// parsing.
    parsed_doc: Option<ConnectedDocument>,
}
...
impl Graph {
    ...
    /// Prepares rewrites for all the paths that have been updated. When a path the graph tracks is
    /// updated, it might introduce new connections to other vertices, whose titles may become out
    /// of date with the present states of those vertices. TODO:
    pub fn prepare_rewrites(&mut self) -> Vec<(PathBuf, String)> {
        let mut rewrites = Vec::new();
        // In addition to all the paths which have been updated, if their titles changed, then we
        // might need to update things that refer to them; those will go in here
        let mut potential_extra_rewrites = HashSet::new();
        // The paths we find for rewriting (which should be subtracted from
        // `potential_extra_rewrites`, these ones are actually getting rewritten)
        let mut paths_rewritten = HashSet::new();

        // Go through all the paths with parsed documents and check the titles of all their
        // connections
        for (path, path_data) in self.paths.iter_mut() {
            if let Some(mut parsed_doc) = path_data.parsed_doc.take() {
                paths_rewritten.insert(path);

                let mut doc_conns = parsed_doc.connections_mut();
                // The root vertex has a 1:1 matching of the connections in the document, and it
                // has information on which ones are valid and pointing somewhere
                let root_vertex = self.vertices.get(&path_data.ids[0]).unwrap();
                let mut root_vertex_conns = root_vertex.connections_out();

                while let Some(informed_conn) = root_vertex_conns.next() {
                    let doc_conn = doc_conns
                        .next()
                        // This is a programming error
                        .expect("mismatch between root vertex and document connections");
                    if let ConnectionTarget::Vertex(uuid) = informed_conn.target {
                        // The root vertex tells us this is a valid connection, which means we have
                        // a prescribed title; set that in the document so it will rewrite
                        // correctly
                        doc_conn.title = self.vertices.get(&uuid).unwrap().title();
                    }
                }
                debug_assert!(
                    doc_conns.next().is_none(),
                    "document had more connections than root vertex in rewrite"
                );
                drop(doc_conns);

                // All titles are updated, serialize the document to a string (also preserves the
                // IDs we're currently referring to). To do that, we'll need to work out the format
                // from the path (guaranteed to be `org`, `md`, or `markdown` from the parsing
                // process for vertices).
                let ext = path.extension().unwrap_or_default();
                let format = if ext == "org" {
                    Format::Org
                } else {
                    Format::Markdown
                };
                let document_str = parsed_doc.into_document(format).into_string(format);

                rewrites.push((path.clone(), document_str));
            }
        }
                ...
                PathPatch::VertexOk {
                    ...
                    self.paths.insert(
                        ...
                        PathData {
                            ids: ids.clone(),
                            error: None,
                            // This is the first time we're seeing this path, we'll need to rewrite
                            // links and IDs
                            parsed_doc: Some(parsed_doc),
                        },
                    );
                }
                PathPatch::VertexErr { path, err } => {
                    ...
                    self.paths.insert(
                        ...
                        PathData {
                            ids: Vec::new(),
                            error: Some(err),
                            // We have no parsed document, there was an error
                            parsed_doc: None,
                        },
                    );
                }
        ...
        for modification in modifications {
            match modification {
                PathPatch::VertexOk {
                    ...
                    // Add the parsed document to the path data; we'll need to rewrite links and
                    // IDs
                    let path_data = self.paths.get_mut(&path).unwrap();
                    path_data.parsed_doc = Some(parsed_doc);
